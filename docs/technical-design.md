...
`CodexStorageModel` owns a utility-priority detached scan, cancellation propagation, the latest snapshot, and page expansion/scroll state. Application startup only restores the cached snapshot; it does not scan. The first entry into the Storage page per app launch, or requesting Refresh, starts a scan independently of the database and usage synchronization. One scan runs at a time. The scanner consumes complete UTF-8 lines from the output pipe and publishes live snapshots at most every 200 ms as output arrives. Before a parent total arrives, completed child totals provide a lower bound; the parent total replaces that sum rather than adding to it. Live categories use declaration order and entries use path order to avoid size-driven reordering during scanning. The previous completed snapshot remains visible until the first progress update and is restored on failure or cancellation. Progress snapshots are never cached; only a completed scan replaces the saved snapshot. Canceled scans never publish partial success. The latest snapshot is atomically saved to `storage.json` beside the TokenTick database. Startup restores it when the Codex and configured projectless roots match, then waits for entry into the Storage page before scanning; UI state stays in memory. Older storage snapshots without newly added categories or a projectless root remain displayable until the background scan replaces them; missing categories are not filled with zero estimates. Missing or malformed cache files are ignored and cache write failures do not block live results. Overview displays the same snapshot below model usage (also available with no usage records); the main-sidebar Storage page exposes directory drill-down, copyable paths, and Finder reveal. Last-scan time and Refresh appear at the top right; the refresh button becomes a spinner and the timestamp is replaced by a localized scanning label during scanning.
...
`LimitResetDetector` compares fresh API snapshots for the same known account, plan, main bucket, and five-hour/seven-day window. It keeps a fixed 60-second reset-boundary tolerance, ignores backward boundaries and nonincreasing observation times, and requires prior positive usage. A forward boundary after the old deadline confirms a natural rollover; an early rollover also requires usage to fall from above 1% to at most 5%. Account/login/plan changes and missing windows discard baselines. Detection is in memory and never seeded from display caches, so restarting does not replay an old celebration. Existing login-generation checks reject late API results, and the delivery controller invalidates queued effects on login changes or reminder enable/disable changes.

`ResetReminderController` uses UserNotifications authorization and normal active-level delivery, including foreground banners. It never requests time-sensitive or critical delivery. The reminder preference does not imply system permission; Settings exposes permission denial and retries. `ResetCelebrationController` owns a temporary nonactivating, click-through AppKit panel on the system primary display with SwiftUI Canvas particles. The panel closes after three seconds; Reduce Motion suppresses effects.
