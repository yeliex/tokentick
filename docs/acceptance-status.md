...
| 图标和原生外观 | 已接入定稿 ICNS、品牌明暗图片和模板菜单图标；`.icon` 隔离编译确认明暗资源可区分，见 [系统级应用图标](app-icon.md) | 整张 PNG 原型存在额外外框，未接入产品；需分层后完成 Dock／Finder 实际外观及 macOS 26 验证 |
...
1. 按已确认的 Icon Composer 路径完成分层图标和系统外观验收，消除隔离原型的重复外框；维护任务恢复已按独立测试、进程终止和历史副本验证。
