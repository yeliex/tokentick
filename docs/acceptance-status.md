...
2026-09-10 初次审计执行 `swift test`：98 个测试、17 个 suite 全部通过，日志 `.build/logs/acceptance-core-tests.log`。加入金额重算恢复后再次执行：101 个测试、18 个 suite 全部通过，日志 `.build/logs/reprice-recovery-full-tests.log`。上述测试覆盖的分支不代表未知上游格式和外部环境已经验收。
...
| 长时间统计缓存重建的断点恢复 | 日志和金额重算已有持久进度；`rebuildStatistics` 仍为单事务替换缓存，失败回滚后重跑 | 补齐中间聚合的持久进度与最终原子发布，验证事实变化后的废弃和进程终止恢复；金额重算的恢复与当前不足 1 秒的重建基线不替代此要求 |
...
1. 继续补齐统计缓存重建的持久进度与恢复测试，保持最终缓存发布的原子性。
