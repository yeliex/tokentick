...
| 菜单栏和关闭主窗口后的行为 | 单模板图标；当前账号主／次额度、重置和同步状态；`86d23ca` Debug 构建通过 | Mac 已解锁，但 Computer Use 未取得无窗口时的状态菜单，关闭后从菜单恢复、菜单设置入口及键盘操作仍待实测 |
| 自动同步和恢复运行 | 调度、取消安静期、文件事件测试；隔离 App 自动追加／归档／补扫验证 | 自动同步开关关闭、重新打开设置后的状态保持及恢复开启已通过；真实系统睡眠／唤醒、其余完整设置交互尚未完成 |
...
| 文件与 SQLite 性能 | 冻结 1,539 文件、约 9.17 GB；JSONL／压缩首次扫描约 96 秒、峰值约 71／75 MiB；静态复扫零字节；跨进程读者约 11 ms | App 资源采样已启动并定位 SHM 事件触发自身扫描；过滤修复通过 6 项专项测试与 Release 构建，修改后的资源对比及持续运行仍在验证，见 [App 运行资源](app-performance.md) |
