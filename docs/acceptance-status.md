...
核对日期：2026-09-10。源码基线 `86d23ca`，环境为 Apple M2 Pro、macOS 27、Xcode 27 beta。完整目标仍未完成。
...
2026-09-10 本次重新执行 `swift test`：98 个测试、17 个 suite 全部通过，耗时约 3.1 秒，日志 `.build/logs/acceptance-core-tests.log`。上述测试覆盖的分支不代表未知上游格式和外部环境已经验收。
...
| 长时间维护任务断点恢复 | 日志有持久游标；`repriceUsage` 每批 512 条提交，但 `lastID` 仅在内存，重新调用从头开始；`rebuildStatistics` 单事务替换缓存，失败回滚后重跑 | 补齐长任务的持久进度与恢复，并验证进度和结果同事务提交、来源变化后不能错误沿用旧进度、进程终止后恢复正确。现有约 10 秒重算／不足 1 秒重建的基线不替代此要求 |
...
| 图标和原生外观 | 已接入定稿 ICNS、品牌明暗图片和模板菜单图标；Debug 外观覆盖不进入 Release | Dock／Finder 图标随系统外观自动切换尚未实现／验证；macOS 26 图标资源方案仍需核实 |
...
1. 先补齐可独立开发的长任务持久进度与恢复测试，保留日志、重算和统计重建的不同事务语义。
