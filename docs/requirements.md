...
周期消耗使用全局本地已确认用量，包含未知账号；与顶部仅属于当前登录账号的额度明确区分。API 参考差额不加入总览金额、tokens 或图表。1 天按最近 24 小时、7／30／90 天及一年按滚动天数（年为 365 天）统计，历史总和覆盖全部已保存用量；时间标签跟随系统时区。不能确定发生日期的用量不伪造图表时间点，历史总和中单独提示其数量。
