...
任务行打开请求明细弹窗，详情按钮打开统计详情弹窗；请求的归属、分项 Tokens、实际费率、金额及原始证据可继续查看。关闭弹窗保留主列表的筛选、聚合及分页。总览带入的精确滚动范围用于明细查询，不悄悄扩大为完整自然日。后台刷新不替换已打开的详情快照，异步旧查询不能覆盖新结果。
...
单账号历史只允许最新且尚未结束的周期显示“进行中”，旧周期显示“已结束”。日期范围优先使用已确认的提前重置时间作为旧周期结束，消耗统计边界同步调整；缺失精确恢复观测时采用现有推算边界。按日期筛选和分页不会改变真实周期状态。

周期详情合为一张卡片，包含已使用百分比、进度、Tokens、预估费用和请求次数。已确认提前重置仅在卡片内显示“手动重置”标签，不在卡片下重复显示事件或时间；时间只显示在周期范围中。不展示统计依据、峰值、额度观测次数、原始记录入口或“最后记录不代表……”等说明。内部保留证据和未知语义，不把最后一次使用比例冒充结算值。请求次数统计周期内本地用量明细数量，不能用额度观测次数代替。
...
3. 验证多个历史周周期切换、单账号仅最新周期进行中、提前重置结束边界，以及合并卡片内容；不将本地消耗和费用混称为订阅扣费。
