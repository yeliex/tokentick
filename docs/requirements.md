...
下方提供 **1 天、7 天、30 天、90 天、一年、历史总和** Tab，统一改变以下内容的统计范围：
...
- 模型使用统计：饼图加列表，支持按金额或 tokens 查看构成，列表仅展示模型名称、金额及 token 数。金额饼图只使用已知金额，未知模型显示“其他”，未定价情况不伪装为零金额。
- 总量、费用与输入／输出／思考／缓存分类属于同一张用量卡片，避免分类孤立成独立模块。模型列表限制宽度并压缩行距，最近对话采用更紧凑的列表行。
- 最近对话使用量列表：对话标题、金额、token 数，按周期内最近消耗时间排序；金额和 tokens 只统计所选周期。点击可进入该任务的用量明细并保留时间范围。
...
在同一页面通过 **每日、项目、任务** 分段控件切换聚合方式，保留日期、账号、模型、项目、任务、未知归属、搜索及排序能力。切换聚合方式保留有效筛选条件，显示当前统计范围、总 tokens 和 API 等值金额，结果分页加载。

表格优先显示聚合名称、tokens、金额及必要状态。选中行后在右侧检查器展示归属、分项用量、模型构成和计价完整性，并支持从项目／每日进入相关任务、从任务进入逐条消耗。明细在主内容区下钻，提供返回路径；选中消耗后在检查器展示实际单价、分项金额、响应／轮次标识与来源证据，JSON 默认折叠。下钻和返回保留统计上下文。

### 8.3 套餐用量

展示历史额度使用记录，当前额度仅在总览展示。历史范围遵循第 6 节：仅主桶 codex 的七天窗口。支持日期和账号筛选，表格显示周期边界、最后观测使用率、观测峰值、周期内本地 tokens 和 API 等值金额；选中周期后在检查器展示首次／最后观测、推算起点、恢复观测及证据，不将最后观测称为最终使用率。
...
主窗口不设底部状态栏。上次同步时间独立放在右上工具栏的刷新按钮之前，采用“上次同步 1 分钟前”等相对时间，不放入按钮或共享胶囊背景；同步进度、取消、问题详情与数据状态入口集中在同步弹出区域。页面文案面向使用者，不显示需求说明、实现方式或验收口径；均匀进度差直接显示“结余 3%”或“超前 3%”，不显示“较均匀进度”。必要的金额估算、未知数据和来源说明放在提示或详情中。
