...
extension UsageStore {
    ...
    public func usageReport(_ query: UsageQuery = UsageQuery()) throws -> UsageReport {
        ...
        if try !pool.read({ try Self.statisticsAreCurrent($0, timezone: timezone.identifier) }) {
    ...
    static func readUsageReport(_ query: UsageQuery, timezone: TimeZone, db: Database) throws -> UsageReport {
        ...
        let current = try Self.statisticsAreCurrent(db, timezone: timezone.identifier)
        if !current { StatisticsSQL.prepare(db, timezone: timezone) }
        let source = current ? "" : "WITH statistics(\(StatisticsSQL.columns)) AS (\(StatisticsSQL.aggregate)) "
        ...
        var arguments: StatementArguments = ["timezone": timezone.identifier, "account": query.account.key,
                                             "dimension": dimension, "limit": query.limit, "offset": query.offset]
        ...
        let rows = try Row.fetchAll(db, sql: """
            ...
            ORDER BY \(query.grouping == .day ? "group_value IS NULL, group_value DESC" : "total_tokens DESC, group_value ASC")
        ...
        let unknown = try Int64.fetchOne(db, sql: """
            ...
            """, arguments: ["timezone": timezone.identifier, "account": query.account.key]) ?? 0
        ...
        return UsageReport(timezone: timezone.identifier, grouping: query.grouping, fromDate: query.fromDate,
                           throughDate: query.throughDate, unknownDateTokens: unknown, rows: summaries)
    }
}
