...
extension UsageStore {
    ...
    static func collectTurns(_ usages: [CollectedUsage], session: RolloutEvent.Session?, db: Database) throws
        ...
        for usage in usages {
            ...
            let start = usage.evidence.turnStartedAt.flatMap(RolloutParser.parseDate)?.timeIntervalSince1970 ?? time
            let created = session?.timestamp.flatMap(RolloutParser.parseDate)?.timeIntervalSince1970
